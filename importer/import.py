...
def create_tabular_files(src_path: Optional[str] = 'output') -> None:
    ...
    for file in files:
        with open(file, 'r') as f:
            ...
            df['src'] = file
            df['extractiondate'] = pd.to_datetime(file.stem.rstrip('_parts'),
                                                  format='%Y-%m-%dT%H-%M-%S')
            trg_path = Path(file.parent).joinpath(f"{file.stem}.csv.gz")
            df.to_csv(trg_path, index=False, compression='gzip')
